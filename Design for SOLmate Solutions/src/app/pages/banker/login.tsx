...
export default function BankerLogin() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      setLoading(true);
      console.log("[BankerLogin] Attempting login for username:", username);
      // POST /api/auth/login
      const res = await authLogin(username, password);
      const { token, role } = extractLoginResult(res);
      ...
      console.log("[BankerLogin] Login success, role:", role);
      ...
      <div className="w-full max-w-md">
        ...
        <Card className="border-2">
          <CardHeader className="text-center pb-4">
            ...
            <CardTitle className="text-2xl">Đăng nhập Banker</CardTitle>
          ...
          <CardContent>
            ...
            <div className="mt-4 text-center">
              <p className="text-xs text-muted-foreground">
                Mặc định: username <strong>admin</strong> / password <strong>admin</strong>
              </p>
            </div>
          </CardContent>
        </Card>
